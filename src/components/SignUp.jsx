...
export default function SignUp(){
    ...
    const create = async (data)=>{
        setError('')
        try {
            const signUpDone = await authService.createAccount(data);
            if(signUpDone){
  ...
  <div className="mx-auto w-full max-w-lg bg-white rounded-xl p-10 shadow-lg border border-gray-200">
    ...
    <form onSubmit={handleSubmit(create)} className="mt-6">
      <div className="space-y-6">
        ...
        />
        <Button type="submit" className="w-full py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-all duration-200">
          Create Account
        </Button>
      </div>
    </form>
